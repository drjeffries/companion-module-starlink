...
export function GetConfigFields(self: ModuleInstance): SomeCompanionConfigField[] {
	return [
		...
		},
		{
			...
			value:
				"Per Starlink's published docs (starlink.readme.io/docs/rate-limits): (1) 250 requests/minute per account - a single unified limit with no separate quota per endpoint type, shared across every integration using that account, not just this connection. (2) The OAuth token endpoint allows 1000 authentications per 15 minutes per client IP - this module caches and reuses each token for its full ~15-minute lifetime, so this is essentially never a concern. (3) Starlink recommends syncing to your own database rather than polling frequently if you need low-latency/high-frequency access. (4) Starlink's Telemetry Stream API docs state user terminal/router devices aggregate telemetry internally on a 15-second interval before it reaches Starlink's backend - the Telemetry Cache API this module actually polls draws from that same pipeline but doesn't have its own separately documented refresh cadence, so polling much faster than ~15s likely won't return fresher numbers. At the defaults below (10s management, 15s telemetry) this module uses roughly 34 requests/minute - well under budget.",
		},
		{
			type: 'number',
